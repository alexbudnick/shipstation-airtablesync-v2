...
export async function main() {
  ...
  logger.info('Fetching Airtable records', {
    ...
    skuField: CFG.fields.sku,
    nameField: CFG.fields.name,
    locationField: CFG.fields.location,
    airtableViewName: CFG.airtableViewName || null
  });
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().catch(err => {
    ...
    logger.error('Fatal sync error - full details below', described);
    console.error('FATAL_SYNC_ERROR_JSON=' + JSON.stringify(described));
    process.exit(1);
  });
}
